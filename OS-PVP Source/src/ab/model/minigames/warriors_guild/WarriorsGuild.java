package ab.model.minigames.warriors_guild;
			...
			if(player.getItems().playerHasItem(8851, 200)) {
				...
				if (current == -1) {
					player.getDH().sendNpcChat4(
							...
							"The fee for entering the area is 200 tokens.",
							"Do you want to enter?", 2461, "Kamfreena");
					player.nextChat = 627;
				} else {
					player.getDH().sendNpcChat3(
							"You are currently in posession of a "+ItemAssistant.getItemName(current)+".",
							"It will cost 200 tokens to re-enter the cyclops area.", 
							"Do you want to enter? It will cost you.", 2461, "Kamfreena");
					player.nextChat = 627;
				}
			} else {
	...
	public void dropDefender(int x, int y) {
		...
		if(isActive() && Boundary.isIn(player, CYCLOPS_BOUNDARY) && !Boundary.isIn(player, WAITING_ROOM_BOUNDARY) && amount > 1) {
			...
			if (Misc.random(chance) == 0) {
				Server.itemHandler.createGroundItem(player, item, x, y, 2, 1, player.playerId);
				player.sendMessage("The cyclops dropped a "+ItemAssistant.getItemName(item)+" on the ground.", 600000);
			}
		}
	}
