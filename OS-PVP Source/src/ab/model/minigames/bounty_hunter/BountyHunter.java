package ab.model.minigames.bounty_hunter;
...
import ab.model.minigames.bounty_hunter.events.TargetDelay;
import ab.model.minigames.bounty_hunter.events.TargetSelector;
...
public class BountyHunter extends CycleEvent {
	...
	public void upgradePlayerEmblem() {
		...
		if (Objects.nonNull(emblem) && emblem.isPresent()) {
			...
			player.sendMessage("<col=255>Your emblem has been upgraded to "+Misc.capitalize(
					emblem.get().name().toLowerCase().replaceAll("_", " "))+".</col>");
		}
	}
	
	public void dropPlayerEmblem(Player killer) {
		Optional<BountyHunterEmblem> emblem = BountyHunterEmblem.getBest(player, false);
		BountyHunterEmblem.EMBLEMS.forEach(e -> player.getItems().deleteItem2(e.getItemId(), 
				player.getItems().getItemAmount(e.getItemId())));
		if (Misc.random(9) != 0 && killer.getBH().hasTarget() && hasTarget() && killer.playerName.
			...
			if (emblem.isPresent()) {
				Server.itemHandler.createGroundItem(killer, emblem.get().getPreviousOrFirst().getItemId(), player.getX(), player.getY(),
					player.heightLevel, 1, killer.playerId);
			} else {
				if (Misc.random(3) != 0) {
					Server.itemHandler.createGroundItem(killer, BountyHunterEmblem.TIER_1.getItemId(), 
							player.getX(), player.getY(), player.heightLevel, 1, killer.playerId);
				}
			}
		}
	}
	...
	public void teleportToTarget() {
		...
		if (player.playerIndex > 0 && target.playerId == player.playerIndex) {
