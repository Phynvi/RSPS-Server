...
public class Clan {
	...
	public void addMember(Player player) {
		...
		if (whoCanJoin > Rank.ANYONE && !isFounder(player.playerName) && !isAdmin(player.playerName)) {
		...
		player.getPA().sendString("Talking in: <col=FFFF75>" + getTitle() + "</col>", 18139);
		player.getPA().sendString("Owner: <col=FFFFFF>" + Misc.formatPlayerName(getFounder()) + "</col>", 18140);
		player.sendMessage("Now talking in clan channel @blu@" + getTitle() + "@bla@");
	...
	public void updateInterface(Player player) {
		...
		for (int index = 0; index < 100; index++) {
			if (index < activeMembers.size()) {
				player.getPA().sendFrame126("<clan=" + getRank(activeMembers.get(index)) + ">" + Misc.formatPlayerName(activeMembers.get(index)),
						18144 + index);
			} else {
		...
		for (int j = 0; j < PlayerHandler.players.length; j++) {
			if (PlayerHandler.players[j] != null) {
				...
				if ((c != null) && (this.activeMembers.contains(c.playerName)))
					c.sendMessage("/@bla@[@blu@" + getTitle() + "@bla@] <img=" + (paramClient.getRights().getValue() - 1) + ">"
							+ "<clan="+ getRank(c.playerName) + ">@bla@"
							+ Misc.optimizeText(paramClient.playerName) + ": @dre@" + paramString);
			}
		}
	}
	...
	public void setRank(String name, int rank) {
		if (PlayerHandler.getPlayer(name).getRights().isOwner()) {
			sendMessage("You cannot rank OS-PvP administrators!");
			return;
		}
	 ...
	 */
	private static final List<String> CLAN_ADMINISTRATORS = Arrays.asList("Alex", //chris
																		  "Alexander",//abnant
																		  "Greco"/*, //dino
																		  "Dark", 
																	      "Jamian"*/);
	...
	public int getRank(String name) {
		...
		if (!Config.PLACEHOLDER_ECONOMY) {
			...
			if (PlayerSave.isFriend2(getFounder(), name)) {
				return Rank.FRIEND;
			}
		}
		return -1;
	...
	public void banMember(String name) {
		...
		sendMessage("[Attempting to kick/ban '" + Misc.formatPlayerName(name) + "'" + " from this clan channel.]");
	}
	...
	public void handleLootShare(Player c, int item, int amount) {
		...
		if (c.getShops().getItemShopValue(item, 1, c.getItems().getItemSlot(item)) > 1) {
			sendLootShareMessage(c.playerName + " has received " + amount + "x " + ab.model.items.Item.getItemName(item) + ".");
		}
	}
