...
public class ClanManager {
	...
	public Clan getHelpClan() {
		for (int i = 0; i < this.clans.size(); i++) {
			if (clans.get(i).getFounder().equalsIgnoreCase("Alex")) {
				return clans.get(i);
			}
		}

		Clan localClan = read("Alex");
		if (localClan != null) {
			clans.add(localClan);
			return localClan;
		}
		localClan = new Clan("Alex", "Alex");
		clans.add(localClan);
		localClan.save();
		return localClan;
	}
	...
	public void delete(Clan paramClan) {
		...
		File localFile = new File("Data/clan/" + paramClan.getFounder()
				+ ".cla");
		if (localFile.delete()) {
			Player localClient = PlayerHandler.getPlayer(paramClan
					.getFounder());
			if (localClient != null) {
